...
var App = new(function() {
    ...
    function setPage(data) {
        ...
        currentPage.insertAdjacentHTML('afterend', template);

        document.title = browserTitle;
        document.body.className = identifier;

        TweenLite.to(currentPage, 1, {
            x: '-25%',
            opacity: 0,
            ease: Power4.easeInOut
        });
        TweenLite.to(currentPage.nextElementSibling, 1, {
            x: 0,
            ease: Power4.easeInOut,
            onComplete: function() {
                window.scroll(0, 0);
        ...
        initPage(currentPage.nextElementSibling, identifier);
    }

    function initPage(page, identifier) {
        if (identifier === 'home') {
            var projects = page.querySelectorAll('.project');

            [].forEach.call(projects, function(project) {
                [].forEach.call(project.querySelectorAll('a'), function(a) {
        ...
        _revealEls = page.querySelectorAll('.reveal');
    }
    ...
    function watch() {
        if (_revealEls.length > 0) {
            var wHeight = window.innerHeight,
                numShown = 0;

            var visibleEls = [].filter.call(_revealEls, function(el) {
                return el.offsetParent !== null;
            });

            [].forEach.call(visibleEls, function(el, i) {
                if (el.getBoundingClientRect().top < wHeight && false === el.classList.contains('revealed')) {
                    numShown++;
    ...
    function init() {
        var body = document.querySelector('body'),
            ...
            identifier = page.getAttribute('data-identifier');
        ...
        body.addEventListener('click', function(e) {
            ...
            if (null !== el && false === el.hasAttribute('data-external')) {
                loadPage(el.getAttribute('href'), true);
                e.preventDefault();
            }
        });

        initPage(page, identifier);
